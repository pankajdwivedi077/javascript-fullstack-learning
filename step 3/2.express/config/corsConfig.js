...
const configureCors = () => {
    return cors({
        ...
        origin: (origin, callback) => {
            const allowedOrignis = [
                'http://localhost:3000', // local dev
                'https://yourcustomdomain.com' // production domain
            ]
            if(!origin || allowedOrignis.indexOf(origin) !== -1){
        ...
        maxAge: 600, //cache pre flight response for 10 mins (600 seconds) -> avoid sending options request multiple time
